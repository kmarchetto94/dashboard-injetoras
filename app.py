...
def load_data():
    """
    Carrega os dados do arquivo CSV. Garante que as colunas existam e que 
    os tipos de dados corretos sejam aplicados para evitar erros de leitura.
    """
    expected_columns = [
    ...
    # CORREÇÃO APLICADA AQUI: Forçamos colunas de texto/selectbox a serem 'str'
    dtype_mapping = {
        'tag': str,
        'id_dosador': str,
        'tag_ont': str,
        ...
        'status_geral': str, # Garante que "OK" / "Não OK" seja salvo corretamente
        'grupo': str,        # Boa prática para colunas de selectbox
        'status_opcua': str  # Boa prática para colunas de selectbox
    }

    if os.path.exists(DATA_FILE):
        # Usamos fillna('') para converter valores nulos (NaN) em strings vazias
        try:
            df = pd.read_csv(DATA_FILE, dtype=dtype_mapping).fillna('')
            # Verifica se todas as colunas esperadas existem, se não, as cria
...
def check_ping(ip):
    """Verifica a conectividade de um IP com um timeout curto."""
    try:
        if pd.isna(ip) or str(ip).strip() == '': return False
        param = '-n' if platform.system().lower() == 'windows' else '-c'
        command = ['ping', param, '1', '-w', '1', str(ip)] # -w 1 = timeout de 1 segundo
...
if page == "Dashboard Geral":
    def style_status_geral(row):
        """Pinta a linha de verde se o status geral for 'OK'."""
        color = '#D4EDDA' if row['status_geral'] == 'OK' else '' # Corrigido para acessar a coluna
    ...
    if st.button("📡 Verificar Conectividade de Rede (Ping)"):
        ...
        if len(all_ips) > 0:
            ...
            st.info("Nenhum IP cadastrado para verificar.")
    ...
    if search_query:
        ...
        # Garante que a busca seja feita em colunas de texto
        mask = df.astype(str).apply(lambda row: any(search_query in val.lower() for val in row), axis=1)
        df_filtrado = df[mask]
    ...
    if 'ping_results' in st.session_state:
        df_display['Ping Injetora'] = df_display['ip_injetora'].apply(lambda ip: '🟢' if st.session_state.ping_results.get(ip) else ('🔴' if pd.notna(ip) and str(ip).strip() != '' and ip in st.session_state.ping_results else '⚪'))
        df_display['Ping Dosador'] = df_display['ip_dosador'].apply(lambda ip: '🟢' if st.session_state.ping_results.get(ip) else ('🔴' if pd.notna(ip) and str(ip).strip() != '' and ip in st.session_state.ping_results else '⚪'))
        df_display['Ping Coletor'] = df_display['ip_coletor'].apply(lambda ip: '🟢' if st.session_state.ping_results.get(ip) else ('🔴' if pd.notna(ip) and str(ip).strip() != '' and ip in st.session_state.ping_results else '⚪'))
    
    st.dataframe(df_display.style.apply(style_status_geral, axis=1), use_container_width=True)
    st.caption("Status do Ping: 🟢 Online | 🔴 Offline | ⚪ Não Testado / IP não preenchido")
    ...
    if df.empty:
        ...
        # Iterar sobre o dataframe filtrado resetando o índice para o loop de colunas funcionar
        for i, row in df_filtrado.reset_index(drop=True).iterrows():
            ...
            with cols[col_index]:
                with st.container():
                    ...
                    # Usar uma chave única combinando 'btn' e a tag da injetora
    ...
    if modal.is_open() and 'selected_injetora' in st.session_state:
        with modal.container():
            ...
            status_opcua_icon = status_opcua_icon_map.get(data['status_opcua'], ICONS['status_off'])
            status_geral_icon = status_geral_icon_map.get(data['status_geral'], ICONS['status_off'])
            ...
            <div class="modal-body">
                ...
                <div class="info-grid">
                    <div class="info-item"><strong><img src="{ICONS['grupo']}">Anexo:</strong> {data.get('grupo') or 'N/A'}</div>
                    <div class="info-item"><strong><img src="{status_opcua_icon}">Status OPCUA:</strong> {data.get('status_opcua') or 'N/A'}</div>
                    <div class="info-item"><strong><img src="{status_geral_icon}">Status Geral:</strong> {data.get('status_geral') or 'N/A'}</div>
                    <div class="info-item"><strong><img src="{ICONS['dependency']}">Dependente Syneco:</strong> {data.get('dependente_syneco') or 'N/A'}</div>
    ...
    tag_options = [""] + sorted([tag for tag in df['tag'].astype(str).unique() if tag and pd.notna(tag)])
    ...
    if st.button("Salvar Alterações", type="primary"):
        # Garante que a coluna 'tag' é do tipo string e remove espaços em branco
        edited_df['tag'] = edited_df['tag'].astype(str).str.strip()
        
        if edited_df['tag'].eq('').any():
            ...
            save_data(edited_df)
            check_ping.clear() # Limpa o cache de ping para forçar a atualização na outra página
