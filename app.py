...
def load_data():
    if os.path.exists(DATA_FILE):
        ...
        return pd.DataFrame(columns=[
            ...
            'tag_ont', 'status_opcua', 'observacoes'
        ])
...
if page == "Dashboard Geral":
    ...
    col3.metric("Não Conectadas (OPCUA)", f"{len(df[df['status_opcua'] == 'Não Conectado'])}")
    ...
    if df.empty:
        ...
        for i, row in df_filtrado.iterrows():
            ...
            with cols[col_index]:
                with st.container():
                    status_icon = status_icon_map.get(row['status_opcua'])
                    card_html = f"""
                    <div style="border: 1px solid #e0e0e0; border-radius: 10px; padding: 15px; box-shadow: 0 4px 6px rgba(0,0,0,0.04); height: 120px; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; background-color: #FFFFFF;">
                        <h3 style="margin: 0 0 10px 0; font-size: 1.5em;">Injetora {row['tag']}</h3>
                        <div style="display: flex; align-items: center; font-size: 1.1em;">
                            <img src="{status_icon}" style="width: 18px; height: 18px; margin-right: 8px;">
                            <span>{row['status_opcua']}</span>
                        </div>
                    </div>
                    """
    ...
    if modal.is_open() and 'selected_injetora' in st.session_state:
        with modal.container():
            ...
            status_icon = status_icon_map.get(data['status_opcua'])
